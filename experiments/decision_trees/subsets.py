...
from experiments.models import DTree
...
description = 'Small decision tree returning 1 if "very good" or "not bad" is present.'

DATASET = 'restaurants'
dir = 'experiments'
model_class = 'decision_trees'
docs = ["This place is not bad. Food is very good!",  # m_very=1
        "This place is not bad. Food is very very good!",  # m_very=2
        "This place is not bad. Food is very very very very very good!",  # m_very=5
        "This place is not bad. Food is very very very very very very good!"]  # m_very=6
...
w_lists = [['not', 'bad'],
           ['very', 'good']]
model = DTree(vectorizer, w_lists)
...
example = data.preprocess(docs[0])
...
for doc in docs:
    ...
    MODEL = 'subsets_m' + str(multiplicities['very'])
    ...
    info = {'Description': description,
            'Model': MODEL, 'Words': w_lists, 'Dataset': DATASET, 'Example': doc, 'N_runs': N_runs,
    ...
    fig.tight_layout()
    fig.suptitle('m_{very}=' + str(info['Multiplicities']['very']), fontsize=fs)
