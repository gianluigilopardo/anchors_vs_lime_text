...
exp_path = os.path.join(results_path, str('exp_logistic_' + DATASET + '.p'))
time_path = os.path.join(results_path, str('time_logistic_' + DATASET + '.p'))
corpus_path = os.path.join(results_path, str('corpus_logistic_' + DATASET + '.p'))
probs_path = os.path.join(results_path, str('probs_logistic_' + DATASET + '.p'))
pickle.dump(corpus, open(corpus_path, 'wb'))
dill.dump(vectorizer, open(os.path.join(results_path, 'vectorizer_logistic_{}.p'.format(DATASET)), 'wb'))
dill.dump(model, open(os.path.join(results_path, 'model_logistic_{}.p'.format(DATASET)), 'wb'))
...
for i, example in np.ndenumerate(corpus):
    ...
    print(anchor_exp)
    lime_t0 = time.time()
    lime_exp = utils.lime_dict(
        lime_explainer.explain_instance(str(example), clf.predict_proba, num_features=len(str(example))))
